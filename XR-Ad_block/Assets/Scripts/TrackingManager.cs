...
{
    [Header("Dependencies")]
    [SerializeField] private DetectionPostProcessor detectionPostProcessor;
    //[SerializeField] private DecisionManager decisionManager;
    
    [Header("Tracking Settings")]
    [SerializeField] private float timeToLive = 2.0f; // Seconds before object expires
    [SerializeField] private float iouThreshold = 0.5f; // Threshold for matching detections
    ...
    */
    public void UpdateOCRResult(TrackedObject obj, string text, bool shouldBlock)
    {
        ...
        if (tracked != null)
        {
            ...
            Debug.Log($"Updated OCR result for object {obj.id}: '{text}', shouldBlock={shouldBlock}");
        ...
        {
            Debug.LogWarning($"Tried to update OCR result for object {obj.id} but it's not in tracked list");
        }
    }
